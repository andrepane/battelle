...
export const COMPACT_LABELS=Object.freeze({personal_social_total:'Personal/Social',adaptativa_total:'Adaptativa',motora_gruesa:'Motora gruesa',motora_fina:'Motora fina',motora_total:'Motora',comunicacion_receptiva:'Comunicación receptiva',comunicacion_expresiva:'Comunicación expresiva',comunicacion_total:'Comunicación',cognitiva_total:'Cognitiva',battelle_total:'Battelle total'});
...
export function buildNormalizedResultRow({id,source,model,results,normativeData}){
  ...
  return Object.freeze({id,label:DISPLAY[id]??canonical,canonicalLabel:canonical,type:id==='battelle_total'?'grand-total':AGGREGATES.has(id)?'total':'subarea',pd:value(source?.pd),pc:value(pc?.value),pcKind:pc?.kind??null,z:value(conversion?.ok?conversion.z:null),T:value(conversion?.ok?conversion.T:null),CI:value(conversion?.ok?conversion.CI:null),ECN:value(conversion?.ok?conversion.ECN:null),equivalentAge:value(equivalent),equivalentAgeTechnicalError:source?.equivalentAge&&!source.equivalentAge.ok&&source.equivalentAge.status!=='no_aplicable'?(source.equivalentAge.error??source.equivalentAge.status??'error técnico'):null,provenance:{pc,conversion:conversion?.ok?{table:conversion.table,source:conversion.provenance}:null,equivalentAge:source?.equivalentAge?.ok?{table:source.equivalentAge.table,source:source.equivalentAge.provenance}:null},technicalError:pc&&!conversion?.ok?conversion.error:null});
}
