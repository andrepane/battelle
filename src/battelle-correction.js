...
import { ageBandForMonths, equivalentAgeForScale, percentileForScale } from './battelle-conversions.js';
...
export function validateCorrectionPrerequisites({ assessment, items }){
  ...
  if(assessment && !assessment.manualAgeOverride && dateCheck.error==='evaluacion_antes_nacimiento') errors.push({type:'evaluacion_antes_nacimiento', message:dateCheck.message});
...
export function buildCorrectionResults({ assessment, scoring, model, percentiles, equivalentAges }){
  ...
  for(const [id,s] of Object.entries(scoring.subareas ?? {})){ const pct=s.pd!==null ? pctStatus(percentileForScale({ageMonths,scaleId:id,directScore:s.pd,percentileData:percentiles,requiresReview:s.requiere_revision,maxScore:s.maximo})) : null; subareas[id]={...s, percentile:pct, status:s.completa?'válido':'incompleto'}; }
