...
export function buildCorrectionResults({ assessment, scoring, model, normativeData, dataVersion, modelVersion, correctedAt }){
  const ageMonths=effectiveAgeMonths(assessment); const subareas={}; const scales={}; const warnings=[...(scoring.advertencias??[])]; const errors=[]; const provenance=[];
