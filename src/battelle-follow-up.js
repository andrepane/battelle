...
import { SCORING_RULES_VERSION } from './battelle-scoring.js';
...
export function createFollowUpAssessmentSeed({ previousAssessment, now = new Date(), id }) {
  ...
  return { id, scoringRulesVersion:SCORING_RULES_VERSION.CURRENT, name:previousAssessment.name??'', therapistName:previousAssessment.therapistName??null, birthDate:previousAssessment.birthDate??'', assessmentDate:todayIso(instant), ageMonths:null, manualAgeOverride:false, referenceAssessmentId:previousAssessment.id, observedResponses:{}, observations:{}, correctionMetadata:{}, workflowStatus:'borrador', createdAt:instant.toISOString(), updatedAt:instant.toISOString(), revision:0, deletedAt:null, deletedBy:null, deletionRevision:null };
}
