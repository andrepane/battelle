...
export function toFirestorePayload(record, uid){
  ...
  const {scoringRulesVersion,...compatible}=clean;
  return {...compatible, correctionMetadata:{...compatible.correctionMetadata,scoringRulesVersion}, organizationId:ORGANIZATION_ID};
}
export function createFirestoreAssessmentRepository({user, servicesPromise=getFirebaseServices()}={}){
  ...
  return {
    ...
    async saveAssessment(record, expectedRevision){ const {db,modules}=await ready; const ref=await docRef(record.id); if(ref.id!==record.id) throw invalid(); const payload=toFirestorePayload(record,uid); if(!payload) throw invalid(); return modules.firestore.runTransaction(db, async(tx)=>{ const snap=await tx.get(ref); const current=snap.exists()?fromFirestoreDocument(snap.id,snap.data()):null; if(snap.exists()&&!current) throw invalid(); if(current?.deletedAt) throw deleted(); if(current&&current.scoringRulesVersion!==payload.correctionMetadata.scoringRulesVersion) throw invalid(); if(!current && Number.isInteger(expectedRevision) && expectedRevision>0) throw deleted(); if(current && Number.isInteger(expectedRevision) && current.revision!==expectedRevision){ const e=new Error('La evaluación fue modificada en otro dispositivo.'); e.code=COLLECTION_ERROR.CONFLICT; e.current=current; throw e; } const revision=(current?.revision||0)+1; const saved={...payload,scoringRulesVersion:payload.correctionMetadata.scoringRulesVersion,id:ref.id,createdAt:current?.createdAt||payload.createdAt,updatedAt:new Date().toISOString(),revision,createdBy:current?.createdBy||uid,updatedBy:uid}; const {scoringRulesVersion,...transportSaved}=saved; const firestorePayload={...transportSaved,createdAt:current?snap.data().createdAt:modules.firestore.serverTimestamp(),updatedAt:modules.firestore.serverTimestamp()}; tx.set(ref,firestorePayload); return saved; }); },
