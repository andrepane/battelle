...
import { RESULT_COLUMN_DEFINITIONS, RESULT_COLUMN_PRESETS, selectResultColumns, serializeResultTable } from '../src/battelle-result-table.js';
...
test('conclusión gestiona empates, inválidos, igualdad total e insuficiencia',()=>{const tied=buildDescriptiveSummary({results:results({adaptativa_total:{equivalentAge:eq(30)},comunicacion_total:{equivalentAge:eq(50)}})});assert.match(tied.text,/Las áreas que presentan.*Personal\/Social y Adaptativa/);assert.match(tied.text,/Las áreas con mayor.*Comunicación y Cognitiva/);const same=buildDescriptiveSummary({results:results({personal_social_total:{equivalentAge:eq(40)},adaptativa_total:{equivalentAge:eq(40)},motora_total:{equivalentAge:eq(40)},comunicacion_total:{equivalentAge:eq(40)},cognitiva_total:{equivalentAge:eq(40)}})});assert.match(same.text,/presentan la misma edad equivalente/);const insufficient=buildDescriptiveSummary({results:{summary:{ageMonths:48},scales:{personal_social_total:{equivalentAge:eq(30)},adaptativa_total:{equivalentAge:{ok:false}}}}});assert.equal(insufficient.ok,false);assert.equal(insufficient.text,'No hay suficientes edades equivalentes válidas para generar la conclusión.');});
