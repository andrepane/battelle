...
test('administración no calcula resultados y corregir ejecuta el motor una sola vez',()=>{ let calls=0; const r=runCorrection({assessment:assessment({A2:2,A3:0}),items,model,normativeData,scoreAssessment(){calls++; return okScoring;}}); assert.equal(calls,1); assert.equal(r.ok,true); });
