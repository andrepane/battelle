...
test('administración no calcula resultados y corregir ejecuta el motor una sola vez con su versión',()=>{ let calls=0,version; const current={...assessment({A2:2,A3:0}),scoringRulesVersion:'manual-v2'}; const r=runCorrection({assessment:current,items,model,normativeData,scoreAssessment(...args){calls++;version=args[3];return okScoring;}}); assert.equal(calls,1); assert.equal(version,'manual-v2'); assert.equal(r.ok,true); });

test('discrepancia_basal permite corregir y permanece en el resultado final',()=>{const warning={tipo:'discrepancia_basal',codigo:'A1',subarea:'Personal/Social|Interacción',mensaje:'Observada inferior al basal.'};const scoring={...okScoring,advertencias:[warning],subareas:{s1:{...okScoring.subareas.s1,advertencias:[warning]}}};const r=runCorrection({assessment:assessment({A1:0,A2:2,A3:0}),items,model,normativeData,scoreAssessment:()=>scoring});assert.equal(r.ok,true);assert.deepEqual(r.results.warnings,[warning]);assert.equal(r.results.subareas.s1.pd,4);});

test('ítems derivados por basal y techo no quedan pendientes y una corrección completa produce PD',()=>{ const r=runCorrection({assessment:assessment({A2:2,A3:0}),items,model,normativeData,scoreAssessment:()=>okScoring}); assert.equal(r.pendingReport.total,0); assert.equal(r.results.scales.personal_social_total.pd,4); assert.equal(r.results.summary.derivedByBasal,1); assert.equal(r.results.summary.derivedByCeiling,1); });

test('hueco interior, techo provisional y contradicciones bloquean con lista detallada',()=>{ const bad={...okScoring, inconsistencias:[{tipo:'inconsistencia_basal'}], subareas:{s1:{...okScoring.subareas.s1, pd:null, completa:false, requiere_revision:true, pendientes:['A3'], techo:{confirmado:true,provisional:true}}}, escalas:{personal_social_total:{pd:null,completa:false,pendientes:['A3']}}}; const r=runCorrection({assessment:assessment({A1:2}),items,model,normativeData,scoreAssessment:()=>bad}); assert.equal(r.status,'correccion_bloqueada'); assert.equal(r.results,null); assert.equal(r.pendingReport.items[0].code,'A3'); assert.equal(r.pendingReport.items[0].area,'Personal/Social'); assert.equal(r.pendingReport.items[0].subarea,'Interacción'); assert.match(r.pendingReport.items[0].reason,/techo provisional|contradicción/); });

test('36-95 no falla por percentiles, Battelle total no inventa percentil y error del motor no genera resultados',()=>{ const a={...assessment({A1:2}), manualAgeOverride:true, ageMonths:52}; const r=runCorrection({assessment:a,items,model,normativeData,scoreAssessment:()=>okScoring}); assert.equal(r.ok,true); assert.equal(r.results.scales.personal_social_total.percentile.status,'valido'); assert.equal(r.results.scales.battelle_total.percentile.status,'no_aplicable'); const e=runCorrection({assessment:a,items,model,normativeData,scoreAssessment(){throw new Error('boom')}}); assert.equal(e.results,null); });
...
test('síntesis admite una sola área principal y no contiene diagnósticos',()=>{ const summary=buildDescriptiveSummary({results:{summary:{ageMonths:52},scales:{comunicacion_total:{equivalentAgeLabel:'40–41 meses',equivalentAge:{ok:true,minMonths:40,maxMonths:41}}}}}); assert.equal(summary.ok,true); assert.match(summary.text,/única área.*Comunicación.*40–41 meses/); assert(!`${summary.text} ${summary.note}`.match(/retraso|leve|moderado|grave|diagnóstico/i)); });
...
test('fecha de evaluación anterior al nacimiento bloquea aunque haya o no anulación manual y no ejecuta motor',()=>{
  for (const manualAgeOverride of [false,true]) {
    ...
    const r=runCorrection({assessment:a,items,model,normativeData,scoreAssessment(){calls++; return okScoring;}});
...
test('los recuentos derivados pertenecen a cada subárea, no a toda la evaluación',()=>{
  ...
  const r=runCorrection({assessment:assessment({A2:2,A4:0}),items,model,normativeData,scoreAssessment:()=>scoring});
...
test('baremos ausentes bloquean sin ejecutar motor y cambio normativo invalida huella',()=>{ let calls=0; const a=assessment({A2:2,A3:0}); const blocked=runCorrection({assessment:a,items,model,scoreAssessment(){calls++; return okScoring;}}); assert.equal(calls,0); assert.equal(blocked.status,'correccion_bloqueada'); assert(blocked.errors.some(e=>e.type==='baremos_invalidos')); const fp1=createCorrectionFingerprint({assessment:a,normativeVersion:'norm-a'}); const fp2=createCorrectionFingerprint({assessment:a,normativeVersion:'norm-b'}); assert.notEqual(fp1,fp2); });
