...
function applyPreviousSubarea(subarea){
  ...
  const occupied=Object.keys(before).length>0;const message=occupied?'Esta subárea ya contiene puntuaciones de la evaluación actual. Si continúas, serán sustituidas por las respuestas observadas de la evaluación anterior.':'Copia como respuestas actuales únicamente las puntuaciones que fueron observadas en la evaluación anterior. El basal y el techo se calcularán de nuevo.';
  if(!confirm(message))return;state.subareaUndo[subarea]=before;state.assessment.observedResponses=copyObservedResponsesForSubarea({currentResponses:state.assessment.observedResponses,previousObservedResponses:state.previousAssessment.observedResponses,itemCodes:codes});maybeInvalidateCorrection(true);renderItems();
}
function undoPreviousSubarea(subarea){const before=state.subareaUndo[subarea];if(!before)return;const codes=state.items.filter(item=>item.area===state.activeArea&&item.subarea===subarea).map(item=>item.codigo_canonico);state.assessment.observedResponses=restoreResponsesForSubarea({currentResponses:state.assessment.observedResponses,previousCurrentResponses:before,itemCodes:codes});delete state.subareaUndo[subarea];maybeInvalidateCorrection(true);renderItems();}
...
function renderItems(){ const container=$('itemsContainer'); const bySub=new Map(); for(const item of state.items.filter(i=>i.area===state.activeArea)){ const k=item.subarea; if(!bySub.has(k)) bySub.set(k,[]); bySub.get(k).push(item); } const sections=[]; let index=0; for(const [sub, items] of bySub){ sections.push(el('details',{class:'subarea-card', dataset:{subarea:sub}, open:index===0}, el('summary',{class:'subarea-summary', dataset:{subareaSummary:sub}}, subareaHeader(sub,items)), subareaDetails(sub,items), ...(state.previousAssessment?[el('div',{class:'reference-subarea-actions'},el('button',{type:'button',class:'secondary-button copy-previous-subarea',dataset:{subarea:sub}},'Mantener puntuaciones anteriores'),el('p',{class:'muted'},'Copia solo respuestas observadas; basal y techo se calcularán de nuevo.'),...(state.subareaUndo[sub]?[el('button',{type:'button',class:'link-button undo-previous-subarea',dataset:{subarea:sub}},'Puntuaciones anteriores aplicadas · Deshacer')]:[]))]:[]), el('div',{class:'item-list'}, ...items.map(renderItemRow)), renderSubareaObservation(sub))); index++; } container.replaceChildren(...sections); updateStartingLevelVisuals(); updateResults(); updateVisibleItemsEffective(); }
...
function setComparisonMode(enabled, notice=''){
  state.comparisonMode=enabled; state.comparisonIds=[]; state.comparisonModel=null; state.comparisonNotice=notice;
...
function renderComparison(model){
  ...
  $('comparisonContent').replaceChildren(
    ...
    renderTable(['Área','PD anterior','PD actual','Edad equivalente anterior','Edad equivalente actual'],rows,'results-table clinical-results comparison-table'));
}
async function viewSelectedComparison(){
  ...
  if(records.some(record=>!record)||records.some(record=>record.workflowStatus!==WORKFLOW_STATUS.CORRECTED)||records[0].id===records[1].id){setComparisonMode(false,'Una evaluación seleccionada ya no existe o ha dejado de estar corregida. Selecciona de nuevo.');return;}
