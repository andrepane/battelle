...
import { buildResultTableModel, displayValue, NOT_APPLICABLE } from './src/battelle-result-table.js';
...
function clinicalAge(months){ return Number.isInteger(months)?`${Math.floor(months/12)} años, ${months%12} meses (${months} meses)`:'—'; }
...
function resultsView(){
  ...
  const model=buildResultTableModel({results:r,model:state.model,normativeData:state.normativeData,professional}); state.resultTableModel=model;
  const rows=model.rows.map(row=>el('tr',{class:`result-row ${row.type}`},td(row.label),resultCell(row.pd),resultCell(row.pc),resultCell(row.z),resultCell(row.T),resultCell(row.CI),resultCell(row.ECN),resultCell(row.equivalentAge)));
  return el('section',{class:'results-view'},
    el('header',{class:'clinical-header'},el('div',{},el('p',{class:'eyebrow'},'Neurointegra'),el('h2',{},'Resumen de resultados Battelle')),el('div',{class:'results-actions'},el('button',{type:'button',id:'editScoresBtn',class:'secondary-button'},'Editar puntuaciones'),el('button',{type:'button',id:'downloadPdfBtn',class:'primary-button'},'Descargar PDF'))),
    el('dl',{class:'clinical-metadata'},el('div',{},el('dt',{},'Paciente'),el('dd',{},patientLabel(model.metadata.name))),el('div',{},el('dt',{},'Fecha de nacimiento'),el('dd',{},displayDate(model.metadata.birthDate))),el('div',{},el('dt',{},'Fecha de evaluación'),el('dd',{},displayDate(model.metadata.assessmentDate))),el('div',{},el('dt',{},'Edad cronológica'),el('dd',{},clinicalAge(model.metadata.ageMonths))),el('div',{},el('dt',{},'Fecha de corrección'),el('dd',{},new Date(state.correction.correctedAt).toLocaleString('es-ES'))),el('div',{},el('dt',{},'Profesional'),el('dd',{},professional))),
    renderTable(model.columns,rows,'results-table clinical-results'),clinicalWarnings(model.warnings),
