...
function subareaProgressLabel(subarea, items){ const answered=items.filter(item=>Object.prototype.hasOwnProperty.call(state.assessment?.observedResponses??{}, item.codigo_canonico)).length; return `${subarea} · ${answered}/${items.length} puntuados`; }
...
function renderItems(){ const container=$('itemsContainer'); const bySub=new Map(); for(const item of state.items.filter(i=>i.area===state.activeArea)){ const k=item.subarea; if(!bySub.has(k)) bySub.set(k,[]); bySub.get(k).push(item); } const sections=[]; let index=0; for(const [sub, items] of bySub){ sections.push(el('details',{class:'subarea-card', dataset:{subarea:sub}, open:index===0}, el('summary',{class:'subarea-summary', dataset:{subareaSummary:sub}}, subareaProgressLabel(sub, items)), el('div',{class:'item-list'}, ...items.map(renderItemRow)), renderSubareaObservation(sub))); index++; } container.replaceChildren(...sections); updateResults(); updateVisibleItemsEffective(); }
...
function renderScoreButtons(item){ const hasObs=Object.prototype.hasOwnProperty.call(state.assessment?.observedResponses??{}, item.codigo_canonico); const obs=state.assessment?.observedResponses?.[item.codigo_canonico]; return [null,0,1,2].map(v=>el('button',{type:'button', class:'score-btn', ariaPressed:String(v===null?!hasObs:obs===v), dataset:{code:item.codigo_canonico, score:v===null?'null':String(v)}, ariaLabel:`${item.codigo}: ${v===null?'no administrado':v}`}, v===null?'—':String(v))); }
