...
function subareaProgressLabel(subarea, items){ const result=Object.values(state.score?.subareas??{}).find((s)=>s.area===items[0]?.area&&s.subarea===subarea); const a=result?.administracion; if(!a){ const answered=items.filter(item=>Object.prototype.hasOwnProperty.call(state.assessment?.observedResponses??{}, item.codigo_canonico)).length; return `${subarea} · ${answered}/${items.length} puntuados`; } const basal=a.basal.confirmado?a.basal.rango_edad:'—'; const ceiling=a.techo.confirmado&&!a.techo.provisional?`${a.techo.inicio} + ${a.techo.fin}`:'—'; return `${subarea} · ${a.status} · basal ${basal} · techo ${ceiling} · Obs. ${a.counts.observed} · 2 basal ${a.counts.derivedByBasal} · 0 techo ${a.counts.derivedByCeiling} · pendientes ${a.counts.pending} · ${a.instruction}`; }
...
function updateVisibleItemsEffective(){ const eff=state.score?.respuestas_efectivas ?? {}; for(const node of document.querySelectorAll('[data-effective]')){ const code=node.dataset.effective; const r=eff[code]; node.classList.remove('hidden'); node.textContent=r?.origen==='observado'?`${r.puntuacion} OBS.`:r?.origen==='basal'?'2 BASAL':r?.origen==='techo'?'0 TECHO':'—'; } for(const card of document.querySelectorAll('.item-card')){ const r=eff[card.dataset.code]; card.dataset.origin=r?.origen ?? 'no_administrado'; } }
