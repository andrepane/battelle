...
import { buildResultTableModel, displayValue, NOT_APPLICABLE, RESULT_COLUMN_DEFINITIONS, RESULT_COLUMN_PRESETS, selectResultColumns, serializeResultTable } from './src/battelle-result-table.js';
...
function currentPreset(){const ids=state.resultColumnSelection;return ids.length===RESULT_COLUMN_PRESETS.piat.length&&RESULT_COLUMN_PRESETS.piat.every(id=>ids.includes(id))?'piat':ids.length===RESULT_COLUMN_PRESETS.complete.length?'complete':'custom';}
function columnControls(){const preset=currentPreset();return el('section',{class:'table-customizer',ariaLabel:'Personalizar tabla'},el('div',{class:'customizer-heading'},el('h3',{},'Personalizar tabla'),el('div',{class:'preset-actions',role:'group',ariaLabel:'Formatos de tabla'},el('button',{type:'button',class:'secondary-button table-preset',dataset:{preset:'piat'},ariaPressed:preset==='piat'},'Resumen PIAT'),el('button',{type:'button',class:'secondary-button table-preset',dataset:{preset:'complete'},ariaPressed:preset==='complete'},'Tabla completa'),el('span',{class:'preset-status',ariaLive:'polite'},preset==='custom'?'Personalizada':''))),el('fieldset',{},el('legend',{},'Columnas visibles'),...RESULT_COLUMN_DEFINITIONS.map(column=>el('label',{class:'column-option'},el('input',{type:'checkbox',class:'result-column-toggle',dataset:{columnId:column.id},checked:state.resultColumnSelection.includes(column.id),disabled:column.required,ariaLabel:`Mostrar columna ${column.label}`}),column.label))));}
...
function resultsView(){
  ...
  const base=buildResultTableModel({results:r,model:state.model,normativeData:state.normativeData,professional}); const conclusion=buildDescriptiveSummary({results:r}); const model=Object.freeze({...base,conclusion}); state.resultTableModel=model;
  const columns=selectResultColumns(state.resultColumnSelection); const rows=model.rows.map(row=>el('tr',{class:`result-row ${row.type}`,ariaLabel:row.type==='grand-total'?'Total Battelle':row.type==='total'?`Total ${row.canonicalLabel}`:undefined},...columns.map((column,index)=>index===0?el('th',{scope:'row'},displayValue(row[column.value])):resultCell(row[column.value]))));
