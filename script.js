...
function resultControls(){const chart=state.resultsDisplay==='chart';const formats=chart?[RESULT_FORMATS.piat,RESULT_FORMATS.mainAreas]:Object.values(RESULT_FORMATS);return el('section',{class:`table-customizer ${chart?'chart-customizer':''}`,ariaLabel:chart?'Formato del gráfico':'Personalizar tabla'},el('div',{class:'customizer-heading'},el('h3',{},chart?'Formato del gráfico':'Personalizar tabla')),el('fieldset',{},el('legend',{},'Resultados incluidos'),...formats.map(format=>el('label',{class:'column-option'},el('input',{type:'radio',name:'result-format',class:'result-format-toggle',value:format.id,checked:state.resultFormatId===format.id}),format.label))),...(chart?[]:[el('fieldset',{},el('legend',{},'Columnas visibles'),...RESULT_COLUMN_DEFINITIONS.map(column=>el('label',{class:'column-option'},el('input',{type:'checkbox',class:'result-column-toggle',dataset:{columnId:column.id},checked:state.resultColumnSelection.includes(column.id),disabled:column.required,ariaLabel:`Mostrar columna ${column.label}`}),column.id==='label'?(state.resultFormatId==='complete'?'Área/subárea':'Área'):column.label)))]));}
