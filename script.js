...
async function renderHome(preloaded=null){
  ...
  const filtering=Boolean(($('assessmentSearch')?.value||'').trim())||(!state.trashMode&&$('assessmentFilter')?.value!=='all'); $('assessmentCount').textContent=filtering?`${filtered.length} de ${records.length} evaluaciones`:`${records.length} ${state.trashMode?'eliminadas':'evaluaciones'}`;
  ...
  host.replaceChildren(renderTable(state.trashMode?['Paciente','Evaluación','Estado anterior','Eliminada','Usuario','Acciones']:['Paciente','Nacimiento','Evaluación','Edad','Estado','Última modificación','Acciones'],rows,'assessments-table'));
}
