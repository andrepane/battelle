...
function resultsView(){
  ...
  const rows=model.rows.map(row=>el('tr',{class:`result-row ${row.type}`,ariaLabel:row.type==='grand-total'?'Total Battelle':row.type==='total'?`Total ${row.canonicalLabel}`:undefined},el('th',{scope:'row'},row.label),resultCell(row.pd),resultCell(row.pc),resultCell(row.z),resultCell(row.T),resultCell(row.CI),resultCell(row.ECN),resultCell(row.equivalentAge)));
