...
import { normalizeHistoryNumber, tryNormalizeHistoryNumber, historyNumberLabel, createFollowUpAssessmentSeed } from './src/battelle-history-number.js';
...
function bindAssessment(){ const a=state.assessment; $('historyNumber').value=a.historyNumber??''; $('historyNumber').placeholder=a.historyNumber?'':'Sin asignar'; $('historyNumberError').textContent=''; $('samePatientBtn').disabled=!tryNormalizeHistoryNumber(a.historyNumber); $('samePatientHelp').textContent=$('samePatientBtn').disabled?'Asigna un número de historia antes de crear otra evaluación del mismo paciente.':''; $('patientName').value=a.name; $('therapistName').value=a.therapistName??''; $('therapistError').textContent=''; $('therapistSuggestions').replaceChildren(...therapistSuggestions(state.assessments).map(name=>el('option',{value:name})));  $('birthDate').value=a.birthDate; $('assessmentDate').value=a.assessmentDate; $('manualAgeOverride').checked=!!a.manualAgeOverride; updateAge(); }
function onMeta(e){ const a=state.assessment; if(!a) return; if(e.target.id==='historyNumber'){ let normalized=null; try{ normalized=normalizeHistoryNumber(e.target.value); $('historyNumberError').textContent=''; e.target.setAttribute('aria-invalid','false'); }catch(err){ $('historyNumberError').textContent=err.message; e.target.setAttribute('aria-invalid','true'); return; } a.historyNumber=normalized; e.target.value=normalized??''; $('samePatientBtn').disabled=!normalized; $('samePatientHelp').textContent=normalized?'':'Asigna un número de historia antes de crear otra evaluación del mismo paciente.'; a.updatedAt=new Date().toISOString(); updateResults(); if(normalized&&sanitizeTherapistName(a.therapistName)){ const firstSave=(state.openedRevision??0)===0; scheduleSave(300); if(firstSave) void flushSave().then(result=>{if(result?.ok&&state.assessment?.id===a.id)return subscribeOpenAssessment(a.id);}); } return; } if(e.target.id==='therapistName'){ const canonical=canonicalTherapistName(e.target.value,state.assessments); a.therapistName=canonical; $('therapistError').textContent=canonical?'':'Indica el terapeuta responsable.'; if(canonical) e.target.value=canonical; a.updatedAt=new Date().toISOString(); updateResults(); if(canonical&&tryNormalizeHistoryNumber(a.historyNumber)){ const firstSave=(state.openedRevision??0)===0; scheduleSave(300); if(firstSave) void flushSave().then(result=>{ if(result?.ok&&state.assessment?.id===a.id) return subscribeOpenAssessment(a.id); }); } return; } if(e.target.id==='patientName') a.name=e.target.value; if(e.target.id==='birthDate') a.birthDate=e.target.value; if(e.target.id==='assessmentDate') a.assessmentDate=e.target.value; if(e.target.id==='manualAgeOverride'){ a.manualAgeOverride=e.target.checked; if(a.manualAgeOverride && !Number.isInteger(a.ageMonths)){ const r=calculateAgeMonths(a.birthDate,a.assessmentDate); a.ageMonths=r.ok?r.months:null; } } if(e.target.id==='ageMonths'){ a.manualAgeOverride=true; $('manualAgeOverride').checked=true; const v=e.target.value===''?null:Number(e.target.value); a.ageMonths=Number.isInteger(v)&&v>=0&&v<=95?v:null; } a.updatedAt=new Date().toISOString(); updateAge(); maybeInvalidateCorrection(e.target.id!=='patientName'); if(e.target.id!=='patientName') updateStartingLevelVisuals({scroll:true, reason:'age'}); }
...
function renderItems(){ const container=$('itemsContainer'); const bySub=new Map(); for(const item of state.items.filter(i=>i.area===state.activeArea)){ const k=item.subarea; if(!bySub.has(k)) bySub.set(k,[]); bySub.get(k).push(item); } const sections=[]; let index=0; for(const [sub, items] of bySub){ sections.push(el('details',{class:'subarea-card', dataset:{subarea:sub}, open:index===0}, el('summary',{class:'subarea-summary', dataset:{subareaSummary:sub}}, subareaHeader(sub,items)), subareaDetails(sub,items), el('div',{class:'item-list'}, ...items.map(renderItemRow)), renderSubareaObservation(sub))); index++; } container.replaceChildren(...sections); updateStartingLevelVisuals(); updateResults(); updateVisibleItemsEffective(); }
...
function renderItemRow(item){ return el('article',{class:'item-card item-row', dataset:{code:item.codigo_canonico, min:item.rango_edad_min_meses, max:item.rango_edad_max_meses}}, el('div',{class:'item-main'}, el('div',{class:'item-head'}, el('strong',{},item.codigo), el('span',{class:'item-age'},`· ${item.rango_edad}`)), el('p',{class:'item-text'},item.enunciado), el('p',{class:'effective hidden', dataset:{effective:item.codigo_canonico}}, 'Puntuación derivada pendiente de corrección')), el('div',{class:'score-controls', ariaLabel:`Puntuación de ${item.codigo}`},...renderScoreButtons(item))); }
...
function correctEnabled(){ return state.ready && state.normativeValidation?.ok && state.assessment && Boolean(tryNormalizeHistoryNumber(state.assessment.historyNumber)) && Boolean(sanitizeTherapistName(state.assessment.therapistName)) && Number.isInteger(currentAge()) && Object.keys(state.assessment.observedResponses??{}).length>0; }
...
function adminPanel(message='') { const p=observedProgress(); const counts=areas.map(area=>({area,count:Object.keys(state.assessment?.observedResponses??{}).filter(code=>state.items.find(item=>item.codigo_canonico===code)?.area===area).length})); return el('section',{class:'summary-card admin-summary'},el('div',{class:'admin-main'},el('p',{class:'admin-status'},`${statusLabel()} · ${p.answered} respuestas observadas`),el('button',{type:'button',id:'correctBtn',class:'primary-button',disabled:!correctEnabled()},'Corregir Battelle')),message?el('p',{class:'warning',role:'status'},message):'',state.normativeValidation?.ok===false?el('p',{class:'error',role:'alert'},NORMATIVE_ERROR_MESSAGE):'',el('details',{class:'area-progress'},el('summary',{},'Ver progreso por áreas'),el('ul',{},...counts.map(({area,count})=>el('li',{},`${area}: ${count} observadas.`))))); }
...
async function renderHome(preloaded=null){
  ...
  if(state.trashMode){
    rows=filtered.map(r=>el('tr',{},
      td(historyNumberLabel(r.historyNumber),{dataset:{label:'NH'}}),
    ...
    rows=filtered.map(r=>el('tr',{},
      ...
      el('td',{class:'actions'},
        ...
        el('button',{class:'danger-button delete-assessment',dataset:{id:r.id}},'Mover a la papelera')
      )
    ));
  }
  host.replaceChildren(renderTable(state.trashMode?['NH','Paciente','Terapeuta','Evaluación','Estado anterior','Eliminada','Usuario','Acciones']:['NH','Paciente','Terapeuta','Nacimiento','Evaluación','Edad','Estado','Última modificación','Acciones'],rows,'assessments-table'));
}
async function createSamePatientAssessment(){ const source=state.assessment; if(!tryNormalizeHistoryNumber(source?.historyNumber)){ $('samePatientHelp').textContent='Asigna un número de historia antes de crear otra evaluación del mismo paciente.'; $('historyNumber').focus(); return; } const details=`NH: ${historyNumberLabel(source.historyNumber)}\nPaciente: ${patientLabel(source.name)}\nFecha de nacimiento: ${displayDate(source.birthDate)}\nTerapeuta: ${therapistLabel(source.therapistName)}\n\n¿Crear una nueva Battelle?`; if(!confirm(details)) return; const seed=createFollowUpAssessmentSeed(source); if(!(await guardBeforeLeaving({save:flushSave}))) return; await startNew(); Object.assign(state.assessment,seed); bindAssessment(); updateAge(); updateResults(); const id=state.assessment.id; const result=await flushSave(); if(result?.ok&&state.assessment?.id===id) await subscribeOpenAssessment(id); }
