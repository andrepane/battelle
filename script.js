...
async function createAssessmentWithReference(){
  ...
  if(!previousId||state.evaluationStatus!=='corregida'||!await showConfirmDialog({title:'Crear Battelle con referencia',message:'Se creará una Battelle nueva e independiente usando esta evaluación únicamente como referencia visual.',confirmLabel:'Crear nueva Battelle',cancelLabel:'Cancelar',tone:'info',trigger:$('newWithReferenceBtn')}))return;
  if(!(await guardBeforeLeaving({save:flushSave})))return;
  const previous=await state.repository.getAssessment(previousId);
  if(!previous||previous.workflowStatus!==WORKFLOW_STATUS.CORRECTED){await showMessageDialog({title:'Referencia no disponible',message:'La evaluación anterior ya no está disponible o ha dejado de estar corregida.',tone:'error',closeLabel:'Volver'});return;}
...
async function applyPreviousSubarea(subarea,trigger){
  ...
  if(!await showConfirmDialog({title:'Sustituir puntuaciones',message:'Esta subárea ya contiene puntuaciones actuales. Si continúas, se sustituirán por las respuestas observadas de la evaluación anterior.',confirmLabel:'Sustituir puntuaciones',cancelLabel:'Cancelar',tone:'warning',trigger}))return;
...
function clinicalWarnings(warnings){ if(!warnings.length) return ''; return el('section',{class:'clinical-warnings',ariaLabelledby:'reviewScoresTitle'},el('h3',{id:'reviewScoresTitle'},'Puntuaciones para revisar'),el('ul',{},...warnings.map(w=>el('li',{},w.message)))); }
function resultControls(){const chart=state.resultsDisplay==='chart';const formats=chart?[RESULT_FORMATS.piat,RESULT_FORMATS.mainAreas]:Object.values(RESULT_FORMATS);return el('section',{class:`table-customizer ${chart?'chart-customizer':''}`,ariaLabel:chart?'Formato del gráfico':'Personalizar tabla'},el('div',{class:'customizer-heading'},el('h3',{},chart?'Formato del gráfico':'Personalizar tabla')),el('div',{class:'customizer-groups'},el('fieldset',{},el('legend',{},'Formato'),el('div',{class:'option-list'},...formats.map(format=>el('label',{class:'selector-option'},el('input',{type:'radio',name:'result-format',class:'result-format-toggle',value:format.id,checked:state.resultFormatId===format.id}),el('span',{},format.label))))),...(chart?[]:[el('fieldset',{},el('legend',{},'Columnas'),el('div',{class:'option-list'},...RESULT_COLUMN_DEFINITIONS.map(column=>el('label',{class:'selector-option'},el('input',{type:'checkbox',class:'result-column-toggle',dataset:{columnId:column.id},checked:state.resultColumnSelection.includes(column.id),disabled:column.required,ariaLabel:`Mostrar columna ${column.label}`}),el('span',{},column.id==='label'?(state.resultFormatId==='complete'?'Área/subárea':'Área'):column.label)))))])));}
...
function chartPanel(model){const subtitle=model.kind==='comparison'?'Evolución de las edades equivalentes entre ambas evaluaciones.':'Comparación con la edad cronológica en el momento de la evaluación.';const host=el('section',{class:'equivalent-age-chart-panel',dataset:{chartKind:model.kind}},el('header',{class:'chart-panel-header'},el('div',{},el('h3',{class:'sr-only'},'Edades equivalentes'),el('p',{class:'muted'},subtitle)),chartActions(model)),el('div',{class:'chart-host'}));renderEquivalentAgeChart(host.querySelector('.chart-host'),model);return host;}
function resultsView(){
  ...
  const chartAvailable=Boolean(RESULT_FORMATS[state.resultFormatId]?.rowIds); state.equivalentAgeChartModel=chartAvailable?buildEquivalentAgeChartModel({assessments:[{assessment:state.assessment,model}],formatId:state.resultFormatId,patient:state.assessment?.name,title:'Edades equivalentes'}):null;
  ...
  return el('section',{class:'results-view'},
    ...
    el('nav',{class:'result-display-tabs',ariaLabel:'Vista de resultados'},el('button',{type:'button',class:'result-display-toggle',dataset:{display:'table'},ariaPressed:state.resultsDisplay==='table'},'Tabla'),el('button',{type:'button',class:'result-display-toggle',dataset:{display:'chart'},ariaPressed:state.resultsDisplay==='chart'},'Gráfico')),resultControls(),
    ...
    clinicalWarnings(model.warnings),
...
function renderComparison(model){
  ...
  const content=state.comparisonDisplay==='chart'?chartPanel(chartModel):renderTable(['Área','PD anterior','PD actual','Edad equivalente anterior','Edad equivalente actual'],rows,'results-table clinical-results comparison-table');
  $('comparisonContent').replaceChildren(
    ...
    el('nav',{class:'result-display-tabs comparison-display-tabs',ariaLabel:'Vista de comparación'},el('button',{type:'button',class:'comparison-display-toggle',dataset:{display:'table'},ariaPressed:state.comparisonDisplay==='table'},'Tabla comparativa'),el('button',{type:'button',class:'comparison-display-toggle',dataset:{display:'chart'},ariaPressed:state.comparisonDisplay==='chart'},'Gráfico comparativo')),
    content);
}
...
async function removeAssessment(id,trigger=document.activeElement){
  const repository=state.repository,rec=await repository.getAssessment(id); if(!rec||assessmentOperations.has(id))return;
  const ok=await showConfirmDialog({title:'Mover a la papelera',message:`¿Quieres mover la evaluación de ${patientLabel(rec.name)} del ${displayDate(rec.assessmentDate)} a la papelera?\n\nPodrás restaurarla más adelante.`,confirmLabel:'Mover a la papelera',cancelLabel:'Cancelar',pendingLabel:'Moviendo…',tone:'warning',trigger,errorMessage:'No se pudo mover la evaluación a la papelera. Comprueba la conexión e inténtalo de nuevo.',onConfirm:async()=>{if(state.repository!==repository||assessmentOperations.has(id))throw new Error('stale-operation');const current=await repository.getAssessment(id);if(!current||current.revision!==rec.revision)throw new Error('stale-assessment');assessmentOperations.add(id);try{await repository.deleteAssessment(id,current.revision);}finally{assessmentOperations.delete(id);}}});
  if(ok){showToast({message:'Evaluación movida a la papelera',tone:'success'});await renderHome();}
}
...
async function restoreDeleted(id,trigger=document.activeElement){const repository=state.repository,rec=(await repository.listDeletedAssessments()).find(r=>r.id===id);if(!rec||assessmentOperations.has(id))return;const ok=await showConfirmDialog({title:'Restaurar evaluación',message:`Se restaurará la evaluación de ${patientLabel(rec.name)} del ${displayDate(rec.assessmentDate)}.`,confirmLabel:'Restaurar',cancelLabel:'Cancelar',pendingLabel:'Restaurando…',tone:'info',trigger,errorMessage:'No se pudo restaurar la evaluación. Comprueba la conexión e inténtalo de nuevo.',onConfirm:async()=>{if(state.repository!==repository||assessmentOperations.has(id))throw new Error('stale-operation');const current=(await repository.listDeletedAssessments()).find(r=>r.id===id);if(!current||current.revision!==rec.revision)throw new Error('stale-assessment');assessmentOperations.add(id);try{await repository.restoreAssessment(id,current.revision);}finally{assessmentOperations.delete(id);}}});if(ok){showToast({message:'Evaluación restaurada',tone:'success'});await renderHome();}}
async function purgeDeleted(id,trigger=document.activeElement){const repository=state.repository,rec=(await repository.listDeletedAssessments()).find(r=>r.id===id);if(!rec||assessmentOperations.has(id))return;const ok=await showConfirmDialog({title:'Eliminar definitivamente',message:'Esta evaluación se eliminará de forma permanente y no podrá restaurarse.',details:[`${patientLabel(rec.name)} · ${displayDate(rec.assessmentDate)}`],confirmLabel:'Eliminar definitivamente',cancelLabel:'Cancelar',pendingLabel:'Eliminando…',tone:'danger',trigger,errorMessage:'No se pudo eliminar definitivamente. La evaluación continúa en la papelera.',onConfirm:async()=>{if(state.repository!==repository||assessmentOperations.has(id))throw new Error('stale-operation');const current=(await repository.listDeletedAssessments()).find(r=>r.id===id);if(!current||current.revision!==rec.revision)throw new Error('stale-assessment');assessmentOperations.add(id);try{await repository.permanentlyDeleteAssessment(id,current.revision);}finally{assessmentOperations.delete(id);}}});if(ok){showToast({message:'Evaluación eliminada definitivamente',tone:'success'});await renderHome();}}

document.addEventListener('click',async (e)=>{ if(e.target.matches('.result-display-toggle')){state.resultsDisplay=e.target.dataset.display;if(state.resultsDisplay==='chart'&&!RESULT_FORMATS[state.resultFormatId]?.rowIds){state.resultFormatId='piat';state.resultColumnSelection=[...RESULT_FORMATS.piat.defaultColumns];}updateResults();return;} if(e.target.matches('.comparison-display-toggle')){state.comparisonDisplay=e.target.dataset.display;renderComparison(state.comparisonModel);return;} if(e.target.matches('.copy-chart,.download-chart')){const panel=e.target.closest('.equivalent-age-chart-panel'),svg=panel?.querySelector('svg'),status=panel?.querySelector('.chart-copy-status'),model=state.equivalentAgeChartModel;if(!svg||!model)return;try{const blob=await chartPngBlob(svg);if(e.target.matches('.download-chart')){downloadChartBlob(blob,safeChartFilename(model));status.textContent='PNG descargado.';}else{try{await copyChartPng(blob);status.textContent='Gráfico copiado como imagen.';}catch{downloadChartBlob(blob,safeChartFilename(model));status.textContent='No se pudo copiar la imagen; se descargó el PNG.';}}}catch{status.textContent='No se pudo generar la imagen.';}return;} if(e.target.id==='newWithReferenceBtn')await createAssessmentWithReference(); if(e.target.id==='comparePreviousBtn'){state.comparisonIds=[state.assessment.referenceAssessmentId,state.assessment.id];await viewSelectedComparison('reference');} if(e.target.matches('.copy-previous-subarea'))await applyPreviousSubarea(e.target.dataset.subarea,e.target); if(e.target.matches('.undo-previous-subarea'))undoPreviousSubarea(e.target.dataset.subarea,e.target); if(e.target.id==='compareBattelleBtn') setComparisonMode(true); if(e.target.id==='cancelComparisonBtn') setComparisonMode(false); if(e.target.id==='viewComparisonBtn') await viewSelectedComparison('general'); if(e.target.id==='backToResultsBtn'){state.comparisonIds=[];state.comparisonModel=null;showAssessment();updateResults();} if(e.target.id==='backFromComparisonBtn'){state.comparisonMode=false;state.comparisonIds=[];state.comparisonModel=null;showHome();} if(e.target.id==='changeComparisonBtn'){state.comparisonMode=true;state.comparisonIds=[];state.comparisonModel=null;showHome();} if(e.target.id==='homeNewAssessmentBtn') await startNew(); if(e.target.id==='backToHomeBtn') await backToHome(); if(e.target.id==='signOutBtn') await signOutFlow(); if(e.target.id==='reloadRemoteBtn') await reloadRemoteAssessment(); if(e.target.id==='keepLocalBtn') $('conflictNotice').classList.add('hidden'); if(e.target.id==='importLocalBtn'){ const r=await importLocalAssessments({repository:state.repository}); $('importLocalResult').textContent=`Importadas: ${r.imported}; omitidas: ${r.skipped}; inválidas: ${r.invalid}; conflictivas: ${r.conflicts}.`; await checkLocalImport(); } if(e.target.id==='trashBtn'){state.trashMode=!state.trashMode;await renderHome();} if(e.target.matches('.restore-assessment')) await restoreDeleted(e.target.dataset.id,e.target); if(e.target.matches('.purge-assessment')) await purgeDeleted(e.target.dataset.id,e.target); if(e.target.matches('.open-assessment')) await openAssessment(e.target.dataset.id); if(e.target.matches('.delete-assessment')) await removeAssessment(e.target.dataset.id,e.target); if(e.target.id==='discardLegacyBtn'){ localStorage.removeItem(LEGACY_KEY); localStorage.setItem('battelleAssessmentsV3:migratedV2','done'); $('legacyNotice').classList.add('hidden'); } if(e.target.id==='discardDraftBtn'){ localStorage.removeItem(STORAGE_KEY); $('draftNotice').classList.add('hidden'); await startNew(true); } if(e.target.id==='discardInvalidBtn'){ localStorage.removeItem(STORAGE_KEY); await startNew(true); } if(e.target.id==='correctBtn') openCorrectionPreflight(); if(e.target.id==='preflightCancelBtn') closeCorrectionPreflight(); if(e.target.id==='preflightConfirmBtn') confirmPreflight(); if(e.target.id==='preflightReviewBtn'){const target=$('correctionPreflightDialog')._preflightModel?.firstReviewTarget;closeCorrectionPreflight();gotoReviewTarget(target);} if(e.target.id==='downloadPdfBtn' && state.resultTableModel) downloadBattellePdf(state.resultTableModel); if(e.target.id==='copyTableBtn'&&state.resultTableModel){const status=$('tableCopyStatus');try{const mode=await copyResultTable(state.resultTableModel);status.textContent=mode==='formatted'?'Tabla copiada con formato':'Tabla copiada como texto';}catch{status.textContent='No se pudo copiar automáticamente. Selecciona la tabla y cópiala manualmente.';}} if(e.target.id==='copyConclusionBtn'&&state.resultTableModel) await copyText(state.resultTableModel.conclusion.text,'conclusionCopyStatus','Conclusión copiada'); if(e.target.id==='editScoresBtn'){ state.viewMode=VIEW_MODE.ADMINISTRATION; updateResults(); updateVisibleItemsEffective(); } if(e.target.matches('.goto-item')) gotoItem(e.target.dataset.code); if(e.target.matches('.area-tab')){ state.activeArea=e.target.dataset.area;state.activeSubarea=null;renderAreas();renderItems(); } if(e.target.matches('.score-btn')) setObservedScore(e.target);});
